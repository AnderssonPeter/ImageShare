...
describe("customFetcher JSON parsing", () => {
  it("wraps a JSON success body in { status, data, headers }", async () => {
    expect.assertions(4);
    // Arrange
    resetFetchMock();
...
describe("customFetcher Accept header", () => {
  ...
  it("lets a per-request Accept header override the default", async () => {
    expect.assertions(1);
    // Arrange
    resetFetchMock();
    fetchMock.mockResolvedValueOnce(okJson({}));
...
describe("customFetcher problem+json error", () => {
  it("unwraps a problem+json error into an ApiError with the problem status", async () => {
    expect.assertions(4);
    // Arrange
    resetFetchMock();
    ...
    // Act
    const thrown = await captureError(customFetcher("/content/missing", { method: "GET" }));
...
describe("customFetcher fallback status", () => {
  it("falls back to the HTTP response status when the problem body has no status", async () => {
    expect.assertions(1);
    // Arrange
    resetFetchMock();
...
describe("customFetcher binary", () => {
  it("wraps a non-JSON success response as a Blob in data", async () => {
    expect.assertions(3);
    // Arrange
    resetFetchMock();
    fetchMock.mockResolvedValueOnce(
...
describe("customFetcher signal", () => {
  it("forwards the abort signal and method to fetch", async () => {
    expect.assertions(2);
    // Arrange
    resetFetchMock();
    fetchMock.mockResolvedValueOnce(okJson({}));
