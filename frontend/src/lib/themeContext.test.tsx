...
describe("themeProvider applies the resolved system theme on mount", () => {
  it("leaves the dark class absent when the system prefers light", () => {
    expect.assertions(2);
    // Arrange + Act
...
describe("themeProvider applies a stored override on mount", () => {
  it("keeps the dark class when the override is dark despite a light system", () => {
    expect.assertions(2);
    // Arrange + Act
    setupThemeEnvironment(false);
    document.documentElement.classList.remove("dark");
...
describe("themeProvider follows the system theme when no override is set", () => {
  it("switches to dark when the system changes to dark", () => {
    expect.assertions(2);
    // Arrange
...
describe("themeProvider ignores system changes when an override is set", () => {
  it("stays on the overridden dark theme when the system changes to light", () => {
    expect.assertions(1);
    // Arrange
    const media = setupThemeEnvironment(false);
    document.documentElement.classList.remove("dark");
...
describe("themeProvider setTheme persists the override and applies it", () => {
  it("stores the theme and toggles the dark class", () => {
    expect.assertions(3);
    // Arrange
    setupThemeEnvironment(false);
    document.documentElement.classList.remove("dark");
...
describe("themeProvider setTheme makes subsequent system changes ignored", () => {
  it("keeps the overridden theme after the system flips", () => {
    expect.assertions(2);
    // Arrange
    const media = setupThemeEnvironment(false);
    document.documentElement.classList.remove("dark");
...
describe("themeProvider clearOverride drops the override and re-follows the system", () => {
  it("returns to the system theme and removes the stored value", () => {
    expect.assertions(4);
    // Arrange
    setupThemeEnvironment(false);
