...
const ROW_POSITION_BASE_STYLE: CSSProperties = { position: 'absolute', top: 0, left: 0, width: '100%' }
...
function useAutoload({ rowVirtualizer, rowCount, hasNextPage, isFetchingNextPage, fetchNextPage }: AutoloadOptions): void {
...
export default function ContentGrid({ path }: { path?: string }): React.JSX.Element {
  ...
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isPending } = useFolderContent(path)
  const items = useMemo(
  ...
  useAutoload({ rowVirtualizer, rowCount, hasNextPage: hasNextPage ?? false, isFetchingNextPage, fetchNextPage })
