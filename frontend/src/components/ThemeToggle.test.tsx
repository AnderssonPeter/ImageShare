...
describe('themeToggle reflects the effective theme with no override', () => {
  it(
    'shows the sun and a "switch to dark" label when the system is light',
    () => {
      expect.assertions(1)
      // Arrange + Act
...
describe('themeToggle switches from light to dark on click', () => {
  it(
    'pins a dark override and updates the icon and label',
    () => {
      expect.assertions(3)
      // Arrange
      setupThemeEnvironment(false)
...
describe('themeToggle switches from dark to light on click', () => {
  it(
    'pins a light override and removes the dark class',
    () => {
      expect.assertions(3)
      // Arrange
...
describe('themeToggle reflects a stored override on mount', () => {
  it(
    'shows the moon when an explicit dark override already exists',
    () => {
      expect.assertions(1)
      // Arrange
      setupThemeEnvironment(false)
...
describe('themeToggle requires a ThemeProvider', () => {
  it(
    'throws when rendered without a provider',
    () => {
      expect.assertions(1)
      // Arrange
      setupThemeEnvironment(false)
